    ...
    for row in range(len(grid)):
        for column in range(len(grid[row])):
            cell = Cell(
                ...
                column,
                index=(row, column),
                distance = 0 if grid[row][column] == 'S' else sys.maxsize,
        ...
        for neighbour in get_neighbours(grid, (current.row, current.column)):
            if neighbour not in visited:
                if can_move(get_cell(grid, current.index), get_cell(grid, neighbour)):
                    index[neighbour].distance = get_distance(current, index[neighbour])
...
def can_move(from_value, destination_value):
    ...
    if destination_value == 'S':
        raise Exception('Cannot move back to start')

    if from_value == 'E':
        raise Exception('Cannot move from end')

    if from_value == 'S':
        from_value = 'a'

    if destination_value == 'E':
        destination_value = 'z'

    return ord(destination_value) - ord(from_value) <= 1
